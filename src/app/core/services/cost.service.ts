...
export class CostService {
  ...
  private getPricingFactorsForRegion(serviceType: string, regionCode: string): any {
    ...
    const basePf = baseModel.pricingFactors || {};
    ...
    if (!region || !region.services || !region.services[serviceType]) {
      return basePf;
    }

    const regionPfMultipliers = region.services[serviceType];
    return this.applyMultipliers(basePf, regionPfMultipliers);
  }

  private applyMultipliers(base: any, multipliers: any): any {
    if (base === null || base === undefined) return base;
    if (typeof base === 'number') {
      const mult = typeof multipliers === 'number' ? multipliers : 1.0;
      return base * mult;
    }
    if (typeof base === 'object') {
      const result: any = Array.isArray(base) ? [] : {};
      for (const key of Object.keys(base)) {
        result[key] = this.applyMultipliers(base[key], multipliers?.[key]);
      }
      return result;
    }
    return base;
  ...
  getCostBreakdown(node: ArchitectureNode, region: string = 'us-east-1'): CostBreakdown {
    ...
    const basePf = model?.pricingFactors || {};
